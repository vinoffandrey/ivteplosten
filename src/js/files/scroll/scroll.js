...
export function headerScroll() {
	...
	const headerShowTimer = header.dataset.scrollShow
		...
		: 500;
	const headerAnimate = header.dataset.animate ? header.dataset.animate : 800;
	...
	document.addEventListener("windowScroll", function (e) {
		...
		if (scrollTop >= startPoint) {
			!header.classList.contains("_header-scroll")
				...
				: null;
			if (
				!header.classList.contains("_header-animate") &&
				!header.classList.contains("_header-animated")
			) {
				header.classList.add("_header-animate");
				setTimeout(function () {
